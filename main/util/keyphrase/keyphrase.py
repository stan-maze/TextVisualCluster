...
from util.keyphrase import FONT_PATH
from util.keyphrase import STOPWORDS
import os
...
class KeyPhraseExtraction():
    ...
    # def wordcloud(self, keyphrrase_weight, mask_pic, save_path='./wordcloud.png', with_mask=False):
    
    def wordcloud(self, keyphrrase_weight, mask_pic, save_path, with_mask=True):
        font = FONT_PATH
        mask = mask_pic
...
def generate(text_path, mask_pic, savepath = None):
    ...
    for word in words:
        if word not in stps:
            result.append(word)
    result = [txt for txt in result if txt not in ('\n', ' ')]
    print(result)
    size = len(result)
    text = ''.join(result)
    ...
    key_phrase_extractor.wordcloud(key_phrase, mask_pic, output_filename)
